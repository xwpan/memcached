		...
		const struct sockaddr *addr, socklen_t addrlen)
{
	...
	// pxw
	/*
    FILE *fp = fopen ("/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/server_address", "w+");
	fprintf (fp, "port %d\n", addr_in->sin_port);
	pr_ip(fp, "the address: ", addr_in->sin_addr.s_addr);
...
mtcp_listen(mctx_t mctx, int sockid, int backlog)
{
	...
	ListenerHTInsert(mtcp->listeners, listener); //把listener加入mtcp的listener hash表
...
mtcp_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	if (!accepted->socket) {
		...
		if (!socket) {
			...
			errno = ENFILE;
			/*fprintf (fp, "create new socket error\n");
			fflush (fp);
			fclose (fp);
			*/
			return -1;
...
mtcp_recv(mctx_t mctx, int sockid, char *buf, size_t len, int flags)
{
	...
	rcvvar = cur_stream->rcvvar; //socket状态
	...
	switch (flags) {
	case 0:
		ret = CopyToUser(mtcp, cur_stream, buf, len);//读
		break;
	case MSG_PEEK:
