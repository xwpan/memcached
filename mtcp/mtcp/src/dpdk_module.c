...
/*----------------------------------------------------------------------------*/
void
dpdk_init_handle(struct mtcp_thread_context *ctxt) //每个线程启动init_handle
{
	...
	dpc = (struct dpdk_private_context *)ctxt->io_private_context;//dpc是该线程的私有上下文
	...
	for (j = 0; j < num_devices_attached; j++) {
		...
		for (i = 0; i < MAX_PKT_BURST; i++) {
			dpc->wmbufs[j].m_table[i] = rte_pktmbuf_alloc(pktmbuf_pool[ctxt->cpu]);//wmbuf从该线程的内存池分配
...
dpdk_load_module(void)
{
	...
	if (!CONFIG.multi_process || (CONFIG.multi_process && CONFIG.multi_process_is_master)) {
		for (rxlcore_id = 0; rxlcore_id < CONFIG.num_cores; rxlcore_id++) { //对每个核都初始化内存池,一个核一个mTCP线程
		...
		for (portid = 0; portid < num_devices_attached; portid++) {
			...
			fflush(stdout);
			ret = rte_eth_dev_configure(portid, CONFIG.num_cores, CONFIG.num_cores, &port_conf);//初始化端口
			if (ret < 0)
			...
			for (rxlcore_id = 0; rxlcore_id < CONFIG.num_cores; rxlcore_id++) {
				ret = rte_eth_rx_queue_setup(portid, rxlcore_id, nb_rxd,
							     ...
							     pktmbuf_pool[rxlcore_id]);//为某个port分配队列，以及捆绑在某个socket上，以及该socket上的某个内存池
				if (ret < 0)
	...
	} else { /* CONFIG.multi_process && !CONFIG.multi_process_is_master */ //多进程模式
		for (rxlcore_id = 0; rxlcore_id < CONFIG.num_cores; rxlcore_id++) {
                        char name[20];
                        sprintf(name, "mbuf_pool-%d", rxlcore_id);
