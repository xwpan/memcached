...
{
	// pxw
//	FILE *fp = fopen("/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/RaiseReadEvent", "a+");

	if (stream->socket) {
		if (stream->socket->epoll & MTCP_EPOLLIN) {
			/*
			fprintf (fp, "RaiseReadEventi. stream->socket->ep_data.sockid = %d\n",
							stream->socket->ep_data.sockid);
			fflush (fp);
			fclose (fp);
			*/
...
RaiseWriteEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
	// pxw
	/*
	FILE *fp = fopen("/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/stream.log", "a+");
	fprintf (fp, "RaiseWriteEvent\n");
	fflush (fp);
	fclose (fp);
	*/
		...
		uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport)
{
	// pxw
	/*
	FILE *fp = fopen("/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/CreateTCPStream", "a+");
	fprintf (fp, "CreateTCPStream\n");
	fflush (fp);
	*/
	...
	if (socket) {
		// pxw
		stream->socket = socket; //socket赋给stream->socket
	...
	/*
	fprintf (fp, "CREATED NEW TCP STREAM %d: "
			"%u.%u.%u.%u(%d) -> %u.%u.%u.%u(%d) (ISS: %u)\n", stream->id, 
			sa[0], sa[1], sa[2], sa[3], ntohs(stream->sport), 
			da[0], da[1], da[2], da[3], ntohs(stream->dport), 
			stream->sndvar->iss);
