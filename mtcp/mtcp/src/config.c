...
//static const char *route_file = "/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/src/config/route.conf";   //pxw : using absolute path
//static const char *arp_file = "/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/src/config/arp.conf";

//static const char *route_file = "config/route.conf";
//static const char *arp_file = "conf/arp.conf";
...
//	{
//		fprintf ( fp, "prog absolute path error\n");
//		return 0;
//	}
//	fprintf ( fp, "prog absolute path : %s\n", current_absolute_path );
	FILE *fc;
	...
	fc = fopen ("/home/dcslab/pxw/memcached/mtcp_file/route.conf","r");
	if (fc == NULL) {
		perror("fopen");
...
#define ARP_ENTRY "ARP_ENTRY"
	...
	fc = fopen ("/home/dcslab/pxw/memcached/mtcp_file/arp.conf","r");
	//fc = fopen(arp_file, "r");
	if (fc == NULL) {
		perror("fopen");
...
ParseConfiguration(char *line)
{
	...
	if (strcmp(p, "num_cores") == 0) {
		...
		if(strncmp(q, ALL_STRING, sizeof(ALL_STRING)) == 0) {
			SetInterfaceInfo(q); //设置dpdk网卡信息，调用rte_eal_init()
		} else {
...
LoadConfiguration(const char *fname)
{
	...
	/* set default configuration */  //默认参数
	...
	while (1) {
		...
		if (fgets(optstr, MAX_OPTLINE_LEN, fp) == NULL) //把fp文件中最大MAX_OPTLINE_LEN长度的字符串写入 optstr
			break;
	
	//	fprintf (log,"load config:	fgets optstr : %s\n", optstr );

		p = optstr;

		// skip comment
		if ((temp = strchr(p, '#')) != NULL) //如果是注释行
			*temp = 0;
		// remove front and tailing spaces
		while (*p && isspace((int)*p))
			p++;
		temp = p + strlen(p) - 1; //实际数据行（去掉前面空格）
		while (temp >= p && isspace((int)*temp))
			   *temp = 0;
		...
		if (ParseConfiguration(p) < 0) { //处理这一行的参数，调用SetInterface -> rte_eal_init
			fclose(fp);
			return -1;
		}
	}
