...
/*----------------------------------------------------------------------------*/
static inline void 
HandleApplicationCalls(mtcp_manager_t mtcp, uint32_t cur_ts) //把stream(usr到mtcp的事件queue)中的数据加入到几个list中？
{
	tcp_stream *stream;
	int cnt, max_cnt;
	int handled, delayed;
...
//	FILE * fp = fopen ("/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/pkt-loop", "w+");
	...
	TRACE_CONFIG("CPU %d: mtcp thread running.\n", ctx->cpu);

	ts = ts_prev = 0;
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) { //死循环。。。。
		...
		for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) { //从端口数目的rx ring接受包
		...
		if (mtcp->ep) {
			FlushEpollEvents(mtcp, ts);//唤醒app 线程处理
		}
		...
		for (tx_inf = 0; tx_inf < CONFIG.eths_num; tx_inf++) { //发送包
...
MTCPRunThread(void *arg)
{
	...
	/* I/O initializing */
	mtcp->iom->init_handle(ctx); //mTCP线程绑定内存池
	...
	if (current_iomodule_func == &dpdk_module_func) {
		...
		if (master == cpu) {
			...
			if (pthread_create(&g_thread[cpu], 
					   NULL, MTCPRunThread, (void *)mctx) != 0) { //创建mTCP线程
...
/*----------------------------------------------------------------------------*/
int 
mtcp_init(const char *config_file) //调用load_module
{
	int i;
	int ret;
	// pxw
...
*/
	...
	PrintInterfaceInfo();//打印网卡信息

	ret = SetRoutingTable();

	//fp =fopen("/home/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/log_mtcp", "a+");
