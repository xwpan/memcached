...
SetInterfaceInfo(char* dev_name_list) 
{
	...
	if (current_iomodule_func == &ps_module_func) {
		...
		close(sock);
	} else if (current_iomodule_func == &dpdk_module_func) { //dpdk模块
		...
		num_queues = MIN(CONFIG.num_cores, MAX_CPUS);//队列数 = cpu个数
