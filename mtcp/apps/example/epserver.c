...
SendUntilAvailable(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	if (sv->total_sent >= fcache[sv->fidx].size) {
		...
		if (sv->keep_alive) {
			...
			mtcp_epoll_ctl(ctx->mctx, ctx->ep, MTCP_EPOLL_CTL_MOD, sockid, &ev);//等待下一个请求连接
...
HandleReadEvent(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	rd = mtcp_read(ctx->mctx, sockid, buf, HTTP_HEADER_LEN); //读长度为HTTP_HEADER_LEN的内容到buf
	if (rd <= 0) {
		return rd;
	}
	...
	sv->keep_alive = FALSE;
	if (http_header_str_val(sv->request, "Connection: ", //由客户端请求获得keep alive参数 
	...
	ev.data.sockid = sockid;
	mtcp_epoll_ctl(ctx->mctx, ctx->ep, MTCP_EPOLL_CTL_MOD, sockid, &ev);//修改event类型
...
AcceptConnection(struct thread_context *ctx, int listener)
{
	...
	c = mtcp_accept(mctx, listener, NULL, NULL); //接受客户端请求，建立连接
...
/*----------------------------------------------------------------------------*/
struct thread_context *
InitializeServerThread(int core) //对某个核上的app thread而言，创建一个对应的mTCP thread，启动，与该app thread交互
{
	...
	ctx->mctx = mtcp_create_context(core);//创建该核上的mtcp线程
	if (!ctx->mctx) {
...
CreateListeningSocket(struct thread_context *ctx)
{
	...
	listener = mtcp_socket(ctx->mctx, AF_INET, SOCK_STREAM, 0); //创建socket
	if (listener < 0) {
	...
	unsigned char *caddr = (unsigned char *)&saddr.sin_addr.s_addr;
	fprintf (stderr, "listen addr = %d.%d.%d.%d\n", caddr[0], caddr[1], caddr[2], caddr[3]);
	
	ret = mtcp_bind(ctx->mctx, listener, //bind listen socket
	...
	/* listen (backlog: 4K) */
	ret = mtcp_listen(ctx->mctx, listener, 4096); //监听
	if (ret < 0) {
	...
	mtcp_epoll_ctl(ctx->mctx, ctx->ep, MTCP_EPOLL_CTL_ADD, listener, &ev);//监听。。把自己加入epoll（注意：都绑定同一个mctx，在该mctx的epoll中添加）

	return listener;
}
/*----------------------------------------------------------------------------*/
void *
RunServerThread(void *arg) //服务端app线程的routine
{
	int core = *(int *)arg;
	struct thread_context *ctx;
	mctx_t mctx;
	...
	/* initialization */
	ctx = InitializeServerThread(core);//mTCP在这里创建，初始化、运行
	if (!ctx) {
	...
	listener = CreateListeningSocket(ctx);//创建监听socket
	if (listener < 0) {
		TRACE_ERROR("Failed to create listening socket.\n");
		exit(-1);
	}

	while (!done[core]) { //usr app thread死循环：
		...
		for (i = 0; i < nevents; i++) {
			...
			fprintf (stderr, "events.data.sockid = %d\n", events[i].data.sockid);
			if (events[i].data.sockid == listener) {//1. 接受客户端连接
				/* if the event is for the listener, accept connection */
				do_accept = TRUE;

			} else if (events[i].events & MTCP_EPOLLERR) { //2. mTCP错误
				int err;
			...
			} else if (events[i].events & MTCP_EPOLLIN) { //3. 客户端可读事件
			...
			} else if (events[i].events & MTCP_EPOLLOUT) { //4， 客户端可写事件
...
main(int argc, char **argv)
{
	...
	ret = mtcp_init(conf_file); //载入arp，route table等，启动load_module
	if (ret) {
	...
	for (i = ((process_cpu == -1) ? 0 : process_cpu); i < core_limit; i++) {
		...
		if (pthread_create(&app_thread[i], 
				   NULL, RunServerThread, (void *)&cores[i])) { //创建app thread
