...
while [ $counter -gt 0 ]
do
    echo "/sbin/ifconfig dpdk$(($counter - 1)) 10.$1.$(( $counter-1)).112 netmask 255.255.0.0 up"
    /sbin/ifconfig dpdk$(($counter - 1)) 10.$1.$(($counter-1)).112 netmask 255.255.0.0 up
    let "counter=$counter - 1"
done
   # /sbin/ifconfig dpdk0 192.168.0.5 netmask 255.255.255.0 up
