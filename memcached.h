 ...
 */
// pxw
typedef struct MTCP_USER_THREAD MTCP_USER_THREAD;
...
struct conn {
    ...
    // pxw
    MTCP_USER_THREAD *thread; /* Pointer to the thread object serving this connection */
};

struct MTCP_USER_THREAD {
    pthread_t thread_id;        /* unique ID of this thread */
    //
    struct event_base *base;    /* libevent handle this thread uses */
    struct event notify_event;  /* listen event for notify pipe */
    ...
    logger *l;                  /* logger buffer */
    void *lru_bump_buf;         /* async LRU bump buffer */
    struct thread_context *ctx; //与mtcp相关结构体
    conn** local_conns;
