...
/*mtcp function*/
static void mtcp_conn_close(struct thread_context* ctx, conn *c);
static enum try_read_result mtcp_try_read_network(struct thread_context* ctx, conn *c);
...
// pxw
conn **conns; //为每个worker创建局部conns，每个大小1000
...
static enum transmit_result mtcp_transmit(struct thread_context* ctx, conn *c);
...
static int add_iov(conn *c, const void *buf, int len) {
    ...
    if (IS_UDP(c->transport)) {
        do {
            ...
            m = &c->msglist[c->msgused - 1];
            m->msg_iov[m->msg_iovlen].iov_base = (void *)buf;//msg_iovlen最多是1024
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
    ...
    size_t nkey;
    int i = 0;
    item *it;  // item是存在memcached的实体，不是CQ中的connection item！
    ...
    do {
        while(key_token->length != 0) {
            ...
            it = limited_get(key, nkey, c); //从memcached中读取item
            if (settings.detail_enabled) {
                stats_prefix_record_get(key, nkey, NULL != it);
            }
            if (it) {
...
static void process_command(conn *c, char *command) {
    ...
    if (ntokens >= 3 &&
        ...
        process_get_command(c, tokens, ntokens, false);  //读操作
        ...
        process_update_command(c, tokens, ntokens, comm, false); //写操作
...
static int try_read_command(conn *c) {
    ...
    if (c->protocol == binary_prot) {
        ...
        *el = '\0';
        ...
        process_command(c, c->rcurr); //执行操作
...
static enum try_read_result mtcp_try_read_network(struct thread_context* ctx, conn *c) {
    ...
    while (1) {
        ...
        int avail = c->rsize - c->rbytes;
        res = mtcp_read(ctx->mctx, c->sfd, c->rbuf + c->rbytes, avail);
        if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.bytes_read += res;
            pthread_mutex_unlock(&c->thread->stats.mutex);
...
static enum transmit_result mtcp_transmit(struct thread_context* ctx, conn *c) {
	...
	// call mtcp write
        //res = sendmsg(c->sfd, m, 0);
	res = mtcp_writev(ctx->mctx, c->sfd, m->msg_iov, m->msg_iovlen);
        if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.bytes_written += res;
            pthread_mutex_unlock(&c->thread->stats.mutex);
		...
		mtcp_epoll_ctl(ctx->mctx, ctx->ep, MTCP_EPOLL_CTL_MOD, c->sfd, &ev);
            	/*
		if (!update_event(c, EV_WRITE | EV_PERSIST)) {
                if (settings.verbose > 0)
                    fprintf(stderr, "Couldn't update event\n");
...
// pxw
static int mtcp_read_into_chunked_item(struct thread_context* ctx, conn *c) {
    int total = 0;
    int res;
    assert(c->rcurr != c->ritem);
    ...
    while (c->rlbytes > 0) {
        ...
        if (c->rbytes > 0) {
            ...
            /*  now try reading from the socket */
            res = mtcp_read(ctx->mctx, c->sfd, ch->data + ch->used,
                    (unused > c->rlbytes ? c->rlbytes : unused));
            if (res > 0) {
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.bytes_read += res;
...
// pxw
void mtcp_drive_machine(struct thread_context *ctx, conn** local_conns, int sfd, int writeflag) {

    conn* c = local_conns[sfd];
    bool stop = false;
    int res;

    assert(c != NULL);

    if(writeflag == 0) // 有读事件触发时，让state为conn_read
    { 
    	c->state = conn_read;
    }
	...
	case conn_listening:
	    ...
	    */
        ...
        case conn_read: //从mtcp socket中读取data
            res = IS_UDP(c->transport) ? try_read_udp(c) : mtcp_try_read_network(ctx, c);
        ...
        case conn_parse_cmd:  //解析请求类型，同时执行kv操作, 错误发生->conn_closing
	    // 读操作； conn_mwrite ->往客户端写
	    // 写操作： conn_nread -> 一般是update item的数据，写完后到conn_write
	...
	case conn_new_cmd:  //请求到达，rbyte > 0 ->conn_parse_cmd; else -> conn_waiting
            reset_cmd_handler(c);
            break;

        case conn_nread:  //完成写操作
            if (c->rlbytes == 0) {
                complete_nread(c);
                break;
            }
		...
		// pxw
                ...
                res = mtcp_read(ctx->mctx, sfd, c->ritem, c->rlbytes);
		if (res > 0) {
                    pthread_mutex_lock(&c->thread->stats.mutex);
                    c->thread->stats.bytes_read += res;
                    pthread_mutex_unlock(&c->thread->stats.mutex);
                ...
                res = mtcp_read_into_chunked_item(ctx, c);
                if (res > 0)
                    break;
            }
        ...
        case conn_swallow:
            ...
            /*  now try reading from the socket */
            res =mtcp_read(ctx->mctx, sfd, c->rbuf, c->rsize > c->sbytes ? c->sbytes : c->rsize);
            if (res > 0) {
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.bytes_read += res;
                pthread_mutex_unlock(&c->thread->stats.mutex);
		...
		*/
        ...
        case conn_write:  //把response写出去。消息组装在msgbuf list中
            /*
        ...
        case conn_mwrite: //发送消息，消息组装在msgbuf list中
          if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c) != 0) {
            ...
            switch (mtcp_transmit(ctx, c)) { // 原transmit版本调用sendmsg,mtcp版本使用write 
...
static int server_socket(const char *interface,
	...
	// 绑定监听socket
	    ...
	    // pxw
	    // 启动监听, &&符号的短路特性，当传输方式是tcp才会调用listen()
...
static int server_sockets(int port, enum network_transport transport,
                          FILE *portnumber_file) {
    if (settings.inter == NULL) {  // 默认为NULL
...
int main (int argc, char **argv) {
    ...
    static char *conf_file = "/home/dcslab/pxw/memcached/mtcp_file/mtcp.conf";
    int ret = mtcp_init(conf_file);
    if(!ret)
	    fprintf(stdout, "mtcp_init success\n");
    else	
	    fprintf(stderr, "mtcp_init failed\n");

#if 0
    /* initialize main thread libevent instance */
    main_base = event_init(); //创建libevent主事件base
    ...
    // 主线程对worker线程的等待在这段main函数结尾
    mtcp_memcached_thread_init(settings.num_threads);  //派生worker事件线程

    if (start_assoc_maintenance_thread() == -1) {
        exit(EXIT_FAILURE);
    }

    if (start_lru_crawler && start_item_crawler_thread() != 0) { // 禁用
        fprintf(stderr, "Failed to enable LRU crawler thread\n");
        exit(EXIT_FAILURE);
    }

    if (start_lru_maintainer && start_lru_maintainer_thread() != 0) {  //禁用
        fprintf(stderr, "Failed to enable LRU maintainer thread\n");
        return 1;
    }

    if (settings.slab_reassign &&
        start_slab_maintenance_thread() == -1) {  // 启用
        exit(EXIT_FAILURE);
    }

    // pxw
    // 该线程检查全局conn[]状态，待修改
    if (settings.idle_timeout && start_conn_timeout_thread() == -1) {  //禁用 
        exit(EXIT_FAILURE);
    }

    /* initialise clock event */
    clock_handler(0, 0, 0); // 禁用
	...
	// pxw
	// main thread开启监听，把接收到的连接分发给worker线程
	// 把这段功能放在worker中执行
#if 0
	errno = 0;
        if (settings.port && server_sockets(settings.port, tcp_transport,  //创建sockets启动TCP server
                                           portnumber_file)) {
        ...
        errno = 0;
        if (settings.udpport && server_sockets(settings.udpport, udp_transport, // 一定要UDP server???
                                              portnumber_file)) {
    ...
    if (event_base_loop(main_base, 0) != 0) {  //进入事件loop
        retval = EXIT_FAILURE;
    }
#endif
